...
def check4CCs(codecFile, newFileName, userSpec):
    ...
    for filecode in fileCodes:
        ...
        sentences = (
            str(len(smallSentenceList)) + ": " + str(smallSentenceList).strip("[]")
        )
        ...
        if filecode in csvCodes:
            for i in range(len(codesInCSV)):
                if filecode == codesInCSV[i][0]:
                    if (
                        codesInCSV[i][2] != "unlisted.csv"
                        and codesInCSV[i][2] != "textualcontent.csv"
                    ):
                        if userSpec == codesInCSV[i][1]:
                            registeredSame.append(
                                [
                                    filecode,
                                    userSpec,
                                    codesInCSV[i][1],
                                    codesInCSV[i][2],
                                    codesInCSV[i][3].strip("[]"),
                                    sentences,
                                ]
                            )
                        if userSpec != codesInCSV[i][1]:
                            registeredDiff.append(
                                [
                                    filecode,
                                    userSpec,
                                    codesInCSV[i][1],
                    ...
                    # Needed to add this because, if the same 4cc was in two spec in unlisted.csv. It would show up as in unlisted and missing.
                        ...
                        # the userspec doesnt equal the code's spec and the code in the csv file isn't in any other CSV files.
                            ...
                            userSpec != codesInCSV[i][1]
                            and codesInCSV[i][0] not in csvCodes[0:i]
                            and codesInCSV[i][0] not in csvCodes[i + 1 :]
                        ):
    ...
    for i in range(len(sentencesCurly)):
        if sentencesCurly[i][0] in curlySpecs:
            curlySpecsList.append(
                [sentencesCurly[i][0], userSpec, sentencesCurly[i][1]]
            )
    ...
    with open(newFileName, "w") as newFile:
        ...
        if registeredSame:
            ...
            csvWriter.writerow(
                ["CODE", "INPUT SPEC", "REG SPEC", "CSV FILE", "CSV ROW", "SENTENCES"]
            )
...
if len(sys.argv) < 3:
    ...
    print(
        "Running %s on %s with shortname %s > %s"
        % (scriptname, codecFile, userSpec, newFileName)
    )
    check4CCs(codecFile, newFileName, userSpec)
elif len(sys.argv) == 4:
    scriptname = sys.argv[0]
